...
void* mm_malloc (size_t size) {
  ...
  BlockInfo * ptrFreeBlock = NULL,*next;
 // size_t blockSize;
 ...
 //coalesceFreeBlock(ptrFreeBlock);
  ...
  if (blockSize - reqSize >= MIN_BLOCK_SIZE)
  {
	...
	next = (BlockInfo *)POINTER_ADD(ptrFreeBlock,reqSize);
	next->sizeAndTags = (blockSize - reqSize) | TAG_PRECEDING_USED ;
	*((size_t*)POINTER_ADD(next,blockSize - reqSize - WORD_SIZE)) = next->sizeAndTags;
	removeFreeBlock(ptrFreeBlock);
	insertFreeBlock(next);
	coalesceFreeBlock(next);
  ...
  else
  {
	...
	next = (BlockInfo *)POINTER_ADD(ptrFreeBlock,blockSize);
...
/*
  ...
  next = (BlockInfo *)POINTER_ADD(ptrFreeBlock,SIZE(ptrFreeBlock->sizeAndTags));
 if (!((next->sizeAndTags)&TAG_USED))
 {
	next->sizeAndTags |= TAG_PRECEDING_USED;
  	*((size_t *)POINTER_ADD(next,SIZE(next->sizeAndTags)-WORD_SIZE)) = next->sizeAndTags;
...
*/
 ...
 //examine_heap();
  return (size_t *)POINTER_ADD(ptrFreeBlock,WORD_SIZE);
